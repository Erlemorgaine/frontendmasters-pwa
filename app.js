...
document.addEventListener("DOMContentLoaded", (event) => {
  ...
  window.addEventListener("beforeinstallprompt", (event) => {
    // preventDefault prevents the browser from showing the dialog that you can install the app
    event.preventDefault();
    bipEvent = event;
  });
  ...
  document.querySelector("#btnShare").addEventListener("click", (event) => {
    ...
    // This Share API is a new api, it might not work on every OS.
    // It will open the standard share menu for Android / iOS
    navigator.share({
      title: "Codepad",
      text: notesString,
    });
